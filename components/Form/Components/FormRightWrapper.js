...
const pinata = new PinataSDK({
  ...
  pinataGateway: "magenta-peaceful-dolphin-479.mypinata.cloud",
});


async function pinning_text(text) {
  try {
...
async function pinning_image(image) {
  try {
...
const FormRightWrapper = () => {
  ...
  const uploadFiles = async (e) => {
    ...
    if(Handler.form.story !== "") {
      try {
        const added = await pinning_text(Handler.form.story);
      ...
      if(Handler.image !== null) {
          try {
              const added = await pinning_image(Handler.image);
